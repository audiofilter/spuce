...
int main(int argv, char* argc[]) {
  ...
  ok = Remz.remez(fir_coef, TAPS, 2, e1, f1, w1, remez_type::BANDPASS);
	...
	// Put at negative frequency
	auto tf_taps = transform_complex_fir("COMPLEX_BAND_PASS", fir_coef, -0.125);
	
	// Map from std::vector<> to fir_coeff<> and then to fir<>
  fir_coeff<std::complex<float_type>> RF(TAPS);
	for (int i=0;i<TAPS;i++) RF.settap(i,tf_taps[i]);
	fir<std::complex<double>,std::complex<double>> RFIR(RF);
